...
#ifdef USE_LIB_WEBSERVER
//! 7.16.26 try included one.. see if fixed M5Atom AP webpage not working.. only the header 
#include <WebServer.h>
#else
#include "WebServer.h"
#endif
...
void WIFI_startWebServer() 
{
    ...
    if (_WIFI_SettingMode)
    {
        ...
        WIFI_webServer.on("/settings", []() {
            ...
            s += "<h1>Wi-Fi Settings for ESP-M5 ATOM S3 Device 2026</h1>";
#elif defined(ESP_M5)
            s += "<h1>Wi-Fi Settings for ESP-M5(M5AtomLite) Device 2026</h1>";
#else // ESP_32
...
#ifdef ESP_M5_too_much_for_users  // 7.16.26
            {
...
//#define AP_DEBUG_MODE  (defined in Perferences..)
          ...
          //  s += "WIFI_CREDENTIAL_1:" +  getPreference_mainModule(PREFERENCE_WIFI_CREDENTIAL_1_SETTING)  + "\n";
            ...
            WIFI_webServer.send(200, "text/html", WIFI_makePage((char*)"Wi-Fi & MQTT Settings 2026", s));
        });
...
#ifdef TRY_FEED_FROM_AP_not_bootstrap
        /******** /feed ******************/
...
#define DOIT_IMAGE // 7.26.26 put back .. lots of room
#ifdef DOIT_IMAGE
//#ifdef ESP_M5_ATOM_S3
    //! 7.16.26 Stormy day, seems the memory (89%) and heap are full so this dynamic string is garbage ..
...
#define TRY_SSID_AMPER
    ...
    s += "{\n";
    s += "   var val;\n";
    s += "   var msg = \"Please confirm values - especially Device Name\\n\";\n";
